...
log_train_name=log_tuning
log_err_name=log_err_tuning
pretrained_model=__pretrained/trained_network.pt

echo -e "Hyperparameter tuning"
echo -e "Please monitor the log tuning: $PWD/${log_train_name}.txt\n"
source $PWD/../../env.sh
CUDA_VISIBLE_DEVICES=3 python main.py --model-forward-with-file-name \
       --num-workers 3 --epochs 100 \
       --no-best-epochs 50 --batch-size 16 \
